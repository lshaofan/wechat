...
import (
	...
	"encoding/json"
	"fmt"
	wechatCxt "github.com/lshaofan/wechat/core/miniprogram/context"
...
func (auth *Auth) Code2SessionContext(ctx context.Context, jsCode string) (result ResCode2Session, err error) {
	var res []byte
	if res, err = util.HTTPGetContext(ctx, fmt.Sprintf(code2SessionURL, auth.AppId, auth.AppSecret, jsCode)); err != nil {
		return
	}

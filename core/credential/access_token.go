...
func NewDefaultAccessToken(appID string, appSecret string, Prefix string, cache *cache.Operation) IAccessToken {
...
func (ak *DefaultAccessToken) GetAccessToken(ctx context.Context) (accessToken string, err error) {
	...
	// 设置cache
	ret := ak.cache.Set(key, result.AccessToken, cache.WithExpire(time.Second*time.Duration(expires)))
